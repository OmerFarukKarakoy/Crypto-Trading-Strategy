...
class TechnicalIndicators:
    """Talib kütüphane kullanmadan indikatör hesaplama"""
...
class LSTMModel:
    ...
    def train(self, data, epochs=50, batch_size=32):
        ...
        #LSTM için reshape etme
        X_train = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))
        X_test = X_test.reshape((X_test.shape[0], X_test.shape[1], 1))
        
        #Modeli build ve train etme
    ...
    def predict(self, data):
        ...
        #Inverse transform (ters dönüşüm) uygulama
...
class Strategy3TradingBot:
    ...
    def create_sample_data(self):
        """API başarısız olduğunda örnek veri"""
    ...
    def calculate_indicators(self):
        """Strateji 3'e göre mevcut alım-satım sinyalini alma"""
        if self.data is None or self.data.empty:
            raise ValueError("No data available. Fetch data first.")
        
        # RSI: Period = 14, levels: 35 (aşırı satım), 50 (nötr), 70 (aşırı alım)
        ...
        #Fiyatın EMA'nın üzerine çıktığını tespit etme
        for i in range(1, len(self.data)):
    ...
    def generate_signal_strategy3(self, index):
        """
        1. 🟢 Güçlü Al: RSI <= 35 ve EMA üzerinde ve WT1 > WT2
        2. 🟦 Temkinli Al: EMA üzerinde ve WT1 > WT2 (RSI > 35)
        3. ⚪ Bekle: (EMA üzerinde ve WT1 < WT2) veya (EMA altında ve WT1 > WT2)
        4. 🟧 Temkinli Sat: EMA altında ve WT1 < WT2 (RSI < 70)
        5. 🔴 Güçlü Sat: RSI >= 70 ve EMA altında ve WT1 < WT2
        """
        if index < 60:
            return None
        ...
        # 1. Güçlü Al
        if rsi <= 35 and price_above_ema and wt1 > wt2:
            return 2  # 🟢 Güçlü Al
        # 2. Güçlü Sat
        elif rsi >= 70 and not price_above_ema and wt1 < wt2:
            return -2  # 🔴 Güçlü Sat
        # 3. Temkinli Al
        elif price_above_ema and wt1 > wt2 and rsi > 35:
            return 1  # 🟦 Temkinli Al
        # 4. Temkinli Sat
        elif not price_above_ema and wt1 < wt2 and rsi < 70:
            return -1  # 🟧 Temkinli Sat
        # 5. Bekle
        elif (price_above_ema and wt1 < wt2) or (not price_above_ema and wt1 > wt2):
            return 0  # ⚪ Bekle
        return None
    
    def execute_trade(self, signal, price, timestamp):
        """Sinyale göre işlem gerçekleştirme"""
    ...
    def get_current_signal_strategy3(self):
        """Strateji 3'e dayalı güncel işlem sinyalini alma"""
    ...
    def get_turkish_recommendation_strategy3(self):
        ...
        if technical_signal == 2:
            ...
            explanation = f"RSI ({rsi:.1f} ≤ 35),Fiyat hareketi EMA üzerinde, WaveTrend AL sinyali."
            confidence = "Çok Yüksek"
            risk_level = "Çok Düşük"
        elif technical_signal == 1:
            action = "🟦 TEMKİNLİ AL"
            status = "EMA ve WaveTrend AL sinyali veriyor."
            explanation = f"Fiyat hareketi EMA üzerinde, WaveTrend AL sinyali. RSI onaylamıyor veya dikkate alınmadı."
            confidence = "Orta"
            risk_level = "Orta"
            ...
            explanation = f"RSI ({rsi:.1f} ≥ 70),Fiyat hareketi EMA altında, WaveTrend SAT sinyali."
            confidence = "Çok Yüksek"
            risk_level = "Çok Düşük"
        elif technical_signal == -1:
            action = "🟧 TEMKİNLİ SAT"
            status = "EMA ve WaveTrend SAT sinyali veriyor."
            explanation = f"Fiyat hareketi EMA altında, WaveTrend SAT sinyali. RSI onaylamıyor veya dikkate alınmadı."
            confidence = "Orta"
            risk_level = "Orta"
        else:
            action = "🟧 TEMKİNLİ SAT"
